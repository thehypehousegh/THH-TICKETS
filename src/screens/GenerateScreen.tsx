...
import type { TicketSelection } from '../db/types';
...
export function GenerateScreen({ route, navigation }: Props) {
  ...
  const [type, setType] = useState(event?.types[0]?.label ?? '');
  ...
  const currentType = type || event.types[0]?.label || '';
  const picked = event.types.filter((t) => (qtys[t.label] || 0) > 0);
  const total = event.types.reduce((n, t) => n + (qtys[t.label] || 0), 0);
  ...
  const onGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const selections: TicketSelection[] = event.types
        .filter((t) => (qtys[t.label] || 0) > 0)
        .map((t) => ({ typeLabel: t.label, typeCode: t.code, quantity: qtys[t.label] }));
      const batch = await generateCodes(event.id, person.trim(), selections);
  ...
  return (
    <Screen>
      ...
      <SelectField
        ...
        value={currentType}
        onChange={setType}
        options={event.types.map((t) => ({ label: `${t.label}  (${t.code})`, value: t.label }))}
      />
