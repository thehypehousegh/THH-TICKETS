...
interface Props {
  batches: BatchRecord[];
  onCheckIn: (match: CodeMatch, used: boolean) => Promise<void>;
  onClose: () => void;
}
...
export function ScanScreen({ batches, onCheckIn, onClose }: Props) {
  ...
  const handleScan = ({ data }: { data: string }) => {
    ...
    locked.current = true;
    const found = findCodeExact(batches, data);
  ...
  const toggle = async () => {
    ...
    setBusy(true);
    try {
      await onCheckIn(match, !match.code.usedAt);
      reset();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <View style={styles.fill}>
      ...
      <View style={styles.topBar}>
        <Button variant="secondary" title="Close" onPress={onClose} style={{ minHeight: 40, paddingHorizontal: 12 }} />
